...
export default function ExpertOnboarding() {
  ...
  useEffect(() => {
    const saveDraft = () => {
      ...
      localStorage.setItem('mentor-onboarding-draft', JSON.stringify({
        data: formData,
        step,
  ...
  useEffect(() => {
    ...
    if (draft) {
      try {
        const { data, step: savedStep } = JSON.parse(draft);
  ...
  const onSubmit = async (data: any, goLiveNow: boolean = false) => {
    setIsSubmitting(true);

    try {
      const profileData = {
        ...data,
        is_profile_live: goLiveNow || canGoLive,
      };
      
      const result = await updateExpertProfile(profileData);
      
      if (goLiveNow || canGoLive) {
        toast.success("Profile created successfully and is now live!");
        localStorage.removeItem('mentor-onboarding-draft'); // Clear draft
      } else {
        toast.success("Draft saved! Complete verification to go live.");
      }
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      toast.success("Draft saved! You can continue later from your dashboard.");
    ...
    try {
      switch (step) {
        ...
        case 2:
          ...
          break;
        case 3:
          // Teaching Certification - now optional
          ...
          if (hasNoCertificate) {
            ...
            if (certs && certs.length > 0) {
              ...
              // No certs but didn't check "no certificate" - that's OK, optional
              isValid = true;
            }
          }
          break;
        case 4:
          ...
          break;
        case 5:
          // Profile Description - only headline required, rest optional
          isValid = await form.trigger(["headline"]);
          if (!isValid) {
            toast.error("Please add a headline for your profile");
          }
          break;
        case 6:
      ...
      if (isValid) {
        ...
        // Check if user can go live (completed minimum required steps)
        const basicInfoComplete = form.formState.dirtyFields.firstName && 
                                  form.formState.dirtyFields.category &&
                                  form.formState.dirtyFields.username;
        const identityVerified = form.getValues("verificationStatus") === "verified";
        const hasService = form.getValues("servicePricing") && 
                          Object.values(form.getValues("servicePricing")).some((s: any) => s?.enabled);
        
        if (basicInfoComplete && identityVerified && hasService) {
          setCanGoLive(true);
        }
        ...
        <div className="w-full max-w-5xl">
          ...
          >
            ...
            <CardContent className="p-6 md:p-8 lg:p-10">
              ...
              {/* Go Live Banner - Show after minimum requirements met */}
              {canGoLive && step < totalSteps && (
                <div className="mb-6 p-4 bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-200 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-bold text-green-900">Ready to Go Live! 🎉</h4>
                        <p className="text-sm text-green-700">
                          You've completed the minimum requirements. You can go live now or complete more fields for better visibility.
                        </p>
                      </div>
                    </div>
                    <Button
