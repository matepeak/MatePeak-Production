-- Unify admin settlement behavior for legacy and atomic payout flows.
-- This keeps old function names but makes them processing-aware and payout-aware.
...
BEGIN
  ...
  IF has_payout_table AND v_payout_id IS NOT NULL THEN
    ...
    IF FOUND AND has_mark_success_rpc THEN
      ...
      -- Preserve reviewer metadata for audit trails.
      UPDATE public.withdrawal_requests
      SET
        reviewed_by = admin_user_id,
        notes = COALESCE(notes, public.withdrawal_requests.notes)
      WHERE id = withdrawal_id;
    ELSE
      -- If payout row is missing, complete using withdrawal-only fallback.
      UPDATE public.withdrawal_requests
      SET
        status = 'completed',
        reviewed_at = now(),
        completed_at = now(),
        reviewed_by = admin_user_id,
        transaction_id = COALESCE(transaction_ref, transaction_id),
        notes = COALESCE(notes, public.withdrawal_requests.notes)
      WHERE id = withdrawal_id;
  ...
  IF has_payout_table AND v_payout_id IS NOT NULL THEN
    ...
    IF FOUND AND has_mark_failed_rpc THEN
      SELECT public.mark_mentor_payout_failed(
        payout_record.id,
        COALESCE(NULLIF(rejection_reason, ''), 'Rejected by admin'),
        jsonb_build_object(
